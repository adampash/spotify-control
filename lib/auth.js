...
const SCOPES = ['user-read-private', 'user-read-email', 'playlist-read-private', 'user-top-read', 'user-library-read', 'user-read-birthdate', 'playlist-modify-private', 'user-read-currently-playing', 'user-read-recently-played', 'user-follow-modify', 'user-modify-playback-state', 'user-read-playback-state', 'user-follow-read', 'user-library-modify', 'streaming', 'playlist-modify-public', 'playlist-read-collaborative'];
const REDIRECT_URL = `http://localhost:${PORT}/auth/callback`;
const AUTH_URL = `${SPOTIFY_URL}?client_id=${process.env.CLIENT_ID}&redirect_uri=${REDIRECT_URL}&scope=${SCOPES.join(' ')}&response_type=code`;
...
const objToForm = obj => Reflect.ownKeys(obj).map(key => `${key}=${obj[key]}`).join('&');

const getLocalToken = () => JSON.parse(fs.readFileSync(ACCESS_TOKEN_PATH));

const getAccessToken = async () => await new Promise((resolve, reject) => {
  try {
    ...
    app.get('/auth/callback', async (req, res) => {
      ...
      const _ref = await authReq.json(),
            { error, error_description } = _ref,
            body = _objectWithoutProperties(_ref, ['error', 'error_description']);
...
const refreshAccessToken = () => new Promise(async (resolve, reject) => {
  try {
    ...
    const _ref2 = await authReq.json(),
          { error, error_description } = _ref2,
          body = _objectWithoutProperties(_ref2, ['error', 'error_description']);
    if (error) reject(error_description);
    fs.writeFileSync(ACCESS_TOKEN_PATH, JSON.stringify(_extends({ refresh_token }, body)));
    resolve(body.access_token);
  } catch (e) {
